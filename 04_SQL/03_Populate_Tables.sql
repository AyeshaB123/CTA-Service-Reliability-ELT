...
-- **** IMPORTANT NOTE *****
-- One time use with static sample data, so Stored Procedures are used in the actual pipeline scripts.
...
FROM Live.vw_Live_Latest_Alerts;

-- Populate data in Train Route Dimension Table
INSERT INTO Dim.Train_Route_Dim(TrainRouteID, TrainRouteName, TrainColorCode)
SELECT DISTINCT m.RouteID, m.RouteName, m.RouteColorCode
FROM Staging.Master_Alerts AS m
WHERE m.RecordType = 'Rail'
	AND NOT EXISTS(
	SELECT 1
	FROM Dim.Train_Route_Dim AS t
...
INSERT INTO Dim.Calendar_Dim(DateID)
SELECT DateID
FROM DateSequence
OPTION (MAXRECURSION 200);

UPDATE Dim.Calendar_Dim
...
INSERT INTO Dim.Time_Dim(HourID)
SELECT HourID
FROM TimeSequence
OPTION (MAXRECURSION 200);
...
INSERT INTO Fact.Live_Alerts_Fact(DateID, HourID, SystemwideRouteID, TrainRouteID, BusRouteID, RouteStatus, Time)
SELECT
	FetchedDate,
...
WHERE NOT EXISTS (
	SELECT 1
	FROM Fact.Live_Alerts_Fact AS a
...
SELECT
	...
	RouteStatus, Time
FROM Fact.vw_Live_Alerts_Fact;

-- View: Live Alerts Fact Preview
SELECT
	AlertID,
	DateID, HourID, SystemwideRouteID, TrainRouteID, BusRouteID,
...
SELECT
	...
	DateID, HourID, SystemwideRouteID, TrainRouteID, BusRouteID,
	RouteStatus, Time, AlertSeverityRange, RouteStatusCategory
FROM Fact.vw_Historical_Alerts;
...
SELECT
	...
	MonthName, MonthShortForm, MonthNumber,
	WeekNumber, Weekday, WeekdayShortForm
FROM Dim.vw_Historical_Calendar;
...
Get_Route_Statuses AS(
	...
	INNER JOIN Fact.vw_Live_Alerts_Fact AS l
...
INSERT INTO Dim.Persistent_Train_Alerts_Dim(TrainRouteID, TrainRouteName, RouteStatus, PersistentStatusCategory, SeverityLevel)
...
FROM Dim.Persistent_Train_Alerts_Dim;
...
INSERT INTO Dim.Persistent_Bus_Alerts_Dim(BusRouteID, BusRouteName, RouteStatus, PersistentStatusCategory, SeverityLevel)
...
SELECT
	...
	RouteStatus, PersistentStatusCategory, SeverityLevel
FROM Dim.Persistent_Bus_Alerts_Dim;
...
INSERT INTO Dim.Persistent_Systemwide_Alerts_Dim(SystemwideRouteID, SystemwideRouteName, RouteStatus, PersistentStatusCategory, SeverityLevel)
...
SELECT
	SystemwideRouteID, SystemwideRouteName,
	RouteStatus, PersistentStatusCategory, SeverityLevel
FROM Dim.Persistent_Systemwide_Alerts_Dim;
