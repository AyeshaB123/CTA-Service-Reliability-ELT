...
-- Active Alerts
SELECT COUNT(RecordID) AS "Active Alerts"
...
SELECT
	[Alert Category], COUNT([Alert Category]) AS "Alert Count"
FROM AlertCategory
GROUP BY [Alert Category];


-- Active Alerts & %Active Alerts: Train | Bus | Systemwide
...
SELECT
	...
	COUNT(l.RouteID) AS "Active Alerts",
	CAST(
		COUNT(l.RouteID) * 100.0 / r.[Route Count] AS DECIMAL (6, 1)
		) AS "%Active Alerts by Route Count"
...
GROUP BY
	l.RecordType, r.[Route Count]
ORDER BY
	[%Active Alerts by Route Count] DESC;
...
SELECT
	...
	COUNT(RecordID) AS "Active Alerts"
FROM Live_Latest_Alerts
WHERE RouteStatus <> 'Normal Service'
GROUP BY RecordType, RouteStatus
ORDER BY [Service], [Active Alerts] DESC;
...
SELECT
	...
	RouteStatusCategory AS 'Status Severity Level'
FROM Live_Latest_Alerts
