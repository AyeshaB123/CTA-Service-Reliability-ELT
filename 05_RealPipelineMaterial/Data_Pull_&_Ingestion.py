...
cursor.execute("TRUNCATE TABLE Live.Live_Latest_Alerts")


for route in cleaned_routes:
    cursor.execute(
        """
        INSERT INTO Live.Live_Latest_Alerts (RecordType, RouteID, RouteName, RouteStatus, RouteColorCode, FetchedDate, FetchedTime)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        route["RecordType"],
        route["RouteID"],
...
for route in cleaned_routes:
    cursor.execute(
        """
        INSERT INTO Staging.Master_Alerts (RecordType, RouteID, RouteName, RouteStatus, RouteColorCode, FetchedDate, FetchedTime)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        route["RecordType"],
        route["RouteID"],
