...
IMPORTANT:
# This file is written as the first step in ELT pipeline: Pulling data from the API and loading it into SQL Server.
# While reviewing this project, this file is not meant to be run since sample data files are already provided in a separate folder as "Sample Datasets".
# It is only shared in this repository to show how the pipeline was built.
...
# STEP 1: Getting data from API and saving it as JSON files


# Fetches CTA Route Status data from "Customer Alerts" API
...
DATA_FOLDER = r"C:\a_Final_Portfolio\CTA-WorkingCopy\alerts_raw_data"
...
# Executed after the database and table structures were created in SQL Server
...
cursor.execute("TRUNCATE TABLE LiveT_Alerts")


for route in cleaned_routes:
    cursor.execute(
        """
        INSERT INTO LiveT_Alerts (RecordType, RouteID, RouteName, RouteStatus, RouteColorCode, FetchedDate, FetchedTime)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        route["RecordType"],
        route["RouteID"],
...
for route in cleaned_routes:
    cursor.execute(
        """
        INSERT INTO MasterT_Alerts (RecordType, RouteID, RouteName, RouteStatus, RouteColorCode, FetchedDate, FetchedTime)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        route["RecordType"],
        route["RouteID"],
